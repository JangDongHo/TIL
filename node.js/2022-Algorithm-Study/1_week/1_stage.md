...
> Q. 콘솔이란 무엇인가요? (지연)
>
> - 콘솔이란 실시간으로 자바스크립트를 실행하고, 에러 메시지등을 출력해주는 기능이다.

> Q. trim()이란 무엇인가요? (지연)
>
> - trim() 메서드는 문자열 양 끝의 공백을 제거한다. 공백이란 모든 공백문자(space, tab, NBSP 등)와 모든 개행문자(LF, CR 등)를 의미한다.
> - `NBSP`: Non-breaking space 직역하면, 줄바꿈 없는 공백이라는 뜻이다. 이를 HTML 환경에서 축약해서 나타내는 문자는 `&nbsp;` 이다.
> - `CR(Carriage Return)`: 현재 위치 커서를 맨 앞으로 이동.
> - `LF(Line Feed)`: 줄바꿈.

> Q. 연산자 우선순위
>
> - 연산자 우선순위를 모두 외울 필요는 없으며 필요할 때 찾아보면 된다. 실무에서는 연산자의 계산 순서를 () (괄호)로 명확하게 나타내는 것을 선호한다.
...
> Q. 빠른 A+B 조사
>
> - node.js로 백준 단계별로 풀어보기 3단계(for문) 문제들을 풀 때 for문 내에서 `console.log()`를 하게 되면 시간 초과가 발생한다.
